...
class ScenarioRunner:
    ...
    def _json_list_or_empty(self, response: requests.Response, context: str):
        payload = self._safe_json(response)
        if response.status_code != 200:
            print(f"{context} failed: {response.status_code} - {payload}")
            return []
        if not isinstance(payload, list):
            print(f"{context} returned unexpected payload type: {type(payload).__name__} - {payload}")
            return []
        return payload
    ...
    def ensure_user(self):
        ...
        if register_resp.status_code in (200, 201):
            print(f"Created user {self.customer_id}")
        elif register_resp.status_code == 400:
            print(f"User {self.customer_id} already exists")
        else:
            print(f"User registration failed: {register_resp.status_code} - {self._safe_json(register_resp)}")

        login_resp = self.session.post(
            f"{self.base_url}/auth/login",
            json={"user_id": self.customer_id, "password": "password123"}
        )
        if login_resp.status_code == 200:
            token = self._safe_json(login_resp).get("access_token")
            if token:
                self.session.headers.update({"Authorization": f"Bearer {token}"})
        else:
            print(f"Login failed: {login_resp.status_code} - {self._safe_json(login_resp)}")

        # Protected endpoint check (best-effort)
        response = self.session.get(f"{self.base_url}/users/{self.customer_id}")
        if response.status_code not in (200, 404):
            print(f"User lookup returned: {response.status_code} - {self._safe_json(response)}")

    def ensure_wallet(self):
        """Ensure wallet exists with initial balance."""
        self.ensure_user()
        print(f"Ensuring wallet exists for {self.customer_id}...")
        response = self.session.get(f"{self.base_url}/wallet/{self.customer_id}")
        if response.status_code == 200:
    ...
    def orders_retry(self):
        ...
        try:
            ...
            print(f"Attempt 1 response: {response1.status_code} - {self._safe_json(response1)}")
        ...
        try:
            ...
            print(f"Attempt 2 response: {response2.status_code} - {self._safe_json(response2)}")
        except requests.exceptions.Timeout:
        ...
        response = self.session.get(f"{self.base_url}/orders?customer_id={self.customer_id}")
        orders = self._json_list_or_empty(response, "List orders")
    ...
    def wallet_concurrency(self):
        ...
        initial_response = self.session.get(f"{self.base_url}/wallet/{self.customer_id}")
        ...
        final_response = self.session.get(f"{self.base_url}/wallet/{self.customer_id}")
    ...
    def false_success(self):
        ...
        print(f"Response body: {self._safe_json(response)}")
        
        time.sleep(0.5)
        
        print(f"\nVerifying order persistence...")
        orders_response = self.session.get(f"{self.base_url}/orders?customer_id={self.customer_id}")
        orders = self._json_list_or_empty(orders_response, "List orders")
    ...
    def mixed(self):
        ...
        print("\n=== Final state ===")
        wallet = self.session.get(f"{self.base_url}/wallet/{self.customer_id}").json()
