...
async def register(user: UserCreate, db: Session = Depends(get_db)):
    logger.info("Register attempt for user_id: %s", user.user_id)
...
async def login(form_data: UserLogin, db: Session = Depends(get_db)):
    ...
    logger.info("Login attempt for user_id: %s", form_data.user_id)
    user = authenticate_user(db, form_data.user_id, form_data.password)
    if not user:
        logger.warning("Failed login attempt for user_id: %s", form_data.user_id)
    ...
    logger.info("User logged in successfully: %s", form_data.user_id)
