...
# Logger for services
logger = logging.getLogger("payment-api.services")
def create_user(db: Session, user_data: UserCreate) -> UserResponse:
    logger.info("Attempting to create user: %s", user_data.user_id)
    ...
    if existing:
        logger.warning("User creation failed, already exists: %s", user_data.user_id)
        raise HTTPException(
    ...
    try:
        ...
        db.commit()
        db.refresh(db_user)
        logger.info("User created successfully: %s", db_user.user_id)
    except IntegrityError:
        db.rollback()
        logger.exception("IntegrityError creating user: %s", user_data.user_id)
...
def authenticate_user(db: Session, user_id: str, password: str) -> Optional[User]:
    ...
    if not user.is_active: 
        logger.warning("Authentication failed: inactive user - %s", user_id)
        return None
    if user.hashed_password is None:
        logger.warning("Authentication failed: no password set - %s", user_id)
        return None
    if not verify_password(password, user.hashed_password):
        logger.warning("Authentication failed: incorrect password - %s", user_id)
        return None
...
def debit_wallet(db: Session, customer_id: str, amount: float) -> Wallet:
    logger.info("Debiting wallet: %s, amount: %.2f", customer_id, amount)
    """
    ...
    if wallet.balance < amount:
        logger.warning("Debit failed: insufficient balance for %s", customer_id)
    ...
    db.commit()
    db.refresh(wallet)
    logger.info("Wallet debited successfully: %s, new balance: %.2f", customer_id, wallet.balance)
    return wallet
