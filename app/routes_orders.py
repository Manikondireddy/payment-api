...
logger = logging.getLogger("payment-api.orders")


@router.post("", response_model=OrderResponse, status_code=201)
def create_order(order: OrderCreate, db: Session = Depends(get_db)):
    logger.info("Received order creation request for customer_id=%s", order.customer_id)
    """
    ...
    try:
        ...
        logger.warning("Validation failed for customer_id=%s: %s", order.customer_id, str(e))
        raise HTTPException(status_code=400, detail=str(e))
    except IntegrityError:
        db.rollback()
        logger.warning("Database constraint violated for customer_id=%s", order.customer_id)

        raise HTTPException(status_code=400, detail="Invalid customer_id or order constraints violated")
    except Exception as e:
        logger.exception("Order creation failed for customer_id=%s", order.customer_id)
...
def list_orders(customer_id: str, db: Session = Depends(get_db)):
    logger.info("Listing orders for customer_id=%s", customer_id)

    """List all orders for a customer."""
    orders = services.get_orders_by_customer(db, customer_id)
    logger.info("Returned %d orders for customer_id=%s", len(orders), customer_id)
    return orders
